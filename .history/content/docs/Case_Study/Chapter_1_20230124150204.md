...
On average, it takes less than 0.25 seconds to process around 140k inputs on NVDIA T4 GPU. In comparison, scikit-learn’s TfidfVectorizer takes 1.50 seconds on CPUs. Notice the shape of the matrix: (152684, 8503). In other words, 150k by 8k floats. This is quite large! On another notes, we can also observe that the run time of scikit-learn’s TfidfVectorizer exponetiates as the input data size increases, whereas CUML, the performace is much more stable.
