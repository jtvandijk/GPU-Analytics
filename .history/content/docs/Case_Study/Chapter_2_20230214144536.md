...
* **schedule**: defines the number of steps to wait before starting the profiling, the number of steps to run the profiling for, and the number of steps to repeat the profiling for. In this example, with repeat=2, profiler will record 2 spans, each span consists of 1 wait step, 1, warmup step and 3 active steps. For more information about wait/warmup/active, you can find it [here](https://pytorch.org/tutorials/recipes/recipes/profiler_recipe.html#using-profiler-to-analyze-long-running-jobs#using-profiler-to-analyze-long-running-jobs). 
...
* **Overview**: provides a high-level overview of the profiling results. This includes a breakdown of the time spent in each function, the number of CUDA kernels launched, and the amount of memory allocated.
* **Input Pipeline**: provides a breakdown of the time spent in each function during the data loading process. This includes the time spent in the data loader, the time spent in the data transformation, and the time spent in the data collation.
* **Operators**: provides a breakdown of the time spent in each function during the forward pass of the model. This includes the time spent in the forward pass of each module, the time spent in the forward pass of each operator, and the time spent in the backward pass of each operator.
...
* **PyTorch Profiler**: provides a breakdown of the time spent in each function during the execution of the model. This includes the time spent in the execution of each function, the time spent in the execution of each operator, and the time spent in the execution of each CUDA kernel.
