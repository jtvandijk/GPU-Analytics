...
As we can see from Figure 3, the GPU Utilization is around 9% and CPU Utilization is around 45%. This means that we are not utilizing the GPU to its full potential, and the CPU is doing most of the work. This may be due to size of the data and model - as the data size and parameters of the model are small in this example, the overhead of transferring the data from CPU to GPU is significant. We can further investigate the profiling logs in the command line: 
...
## Comparing model performance with a pre-trained model 


A pre-trained model is a model that has already been trained on a large dataset. This means that the model has already learned the patterns in the data and can be used to make predictions on new data. Pre-trained models are ty
