...
Let's see how well the model perform in our data linkage task. In this step, we will use the cosine similarity result to find the best match for each query. Since we don't have the unique identifier for each address, we will use the index of the query and target matrix as the unique identifier. 
