...
Again, we can repeat the same process as in case study 1 to profile CPU and GPU usage. By changing model to VGG16 using ```vgg11_bn = VGG.to(device=device)```, we can see that the GPU is now being used.
